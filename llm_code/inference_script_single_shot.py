...
def pre_format_prompts(example_data, example_ids: Union[List[int],List[List[int]]],test_data, template: str=ENTAILMENT_TEMPLATE):
    ...
    if isinstance(example_ids[0], int):
        example_ids = [example_ids for _ in range(test_data_length)]
...
def evaluate_model_with_n_examples(model, tokenizer, dataset, n_examples, is_random=True, train_example_id=0):

    # n_examples * n_eval = 550 max for RTX3090ti
    max_multiplier = 180 # 100 #550
    if is_random:
        train_ids = random.sample(range(len(dataset['train'])),n_examples)
    else:
        train_ids = list(range(dataset))

    train_ids_list = [187512,  490753,  130198,  288550,  263793,  22798,  526281,  202500,  411985,  395881,  48578,  319966,  335522,  265836,  135256,  3590,  301907,  77146,  276903,  13726]
    train_ids = [train_ids_list[train_example_id]]
    ...
    test_type_name = "test"
    test_length = len(dataset[test_type_name]) #// 32 # /2 for faster evaluation
    ...
    for i in tqdm(range(max_iterations)):
        ...
        labels = get_predictions_from_llm(model, tokenizer, dataset['train'], dataset[test_type_name], train_ids, eval_ids)
    ...
    y_true = dataset[test_type_name]['label'][:test_length]
    ...
    return total_accuracy, y_true, label_list, train_ids
...
if __name__=="__main__":
    ...
    n_examples = 1
    train_example_id = 5
    for train_example_id in range(0, 16): 
        print(f'Running experiments with {n_examples}-shot prompting')
        accuracy, true_labels, predicted_labels, train_ids = evaluate_model_with_n_examples(
            model, tokenizer, snli_ds, n_examples=n_examples, is_random=True, train_example_id=train_example_id
        )
        ...
        with open(f'results_{n_examples}_{train_example_id}.json', 'w') as f:
            json.dump(obj_,f)
